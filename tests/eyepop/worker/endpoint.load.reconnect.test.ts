...
function prepMockServer(server: MockServer, test_pop_id: string, test_pipeline_id_1: string, test_pipeline_id_2: string) {
    ...
    const getPipelineRoute1 = server.get(`/worker/pipelines/${test_pipeline_id_1}`).mockImplementationOnce(ctx => {
        ctx.status = 200
        ctx.response.headers['content-type'] = 'application/json'
        ctx.body = JSON.stringify({
    ...
    const getPipelineRoute2 = server.get(`/worker/pipelines/${test_pipeline_id_2}`).mockImplementationOnce(ctx => {
        ctx.status = 200
        ctx.response.headers['content-type'] = 'application/json'
        ctx.body = JSON.stringify({
...
describe('EyePopSdk endpoint reconnect', () => {
    ...
    test('EyePopSdk loadFrom reload config on 404', async () => {
        ...
        const { authenticationRoute, popConfigRoute } = prepMockServer(server, test_pop_id, test_pipeline_id_1, test_pipeline_id_2)

        const loadFromRoute1 = server.patch(`/worker/pipelines/${test_pipeline_id_1}/source`).mockImplementationOnce(async ctx => {
            ctx.status = 404
        })
        const loadFromRoute2 = server.patch(`/worker/pipelines/${test_pipeline_id_2}/source`).mockImplementationOnce(async ctx => {
