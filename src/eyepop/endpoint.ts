    ...
    private async currentAccessToken(): Promise<string> {
        ...
        if (!this._token || <number>this._expire_token_time < now) {
            ...
            try {
                ...
                } else if ((this._options.auth as OAuth2Auth).oAuth2 !== undefined && this._options.eyepopUrl && this._options.popId) {
                    const session = await authenticateBrowserSession(((this._options.auth as OAuth2Auth).oAuth2 as Auth0Options), this._options.eyepopUrl, this._options.popId)
