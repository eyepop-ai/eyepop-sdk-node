import { EyepopLineDecoder } from 'EyePop/streaming'
import { parseHttpHeader } from 'EyePop/shims/rn_http_header_parser'
...
export class ResponseParser {
    ...
    constructor(logger: Logger, resolve: (value: Response | PromiseLike<Response>) => void, reject: (reason?: any) => void) {
        this.logger = logger
    ...
    public processResponseReadBuffer(data: Buffer): boolean {
        if (this.error) {
            this.logger.info(`RN http streamed response, received ${data.length} bytes in error mode`);
            return false;
        }
        ...
        if (this.responseState === ResponseState.RECEIVING_COMPLETED) {
            ...
            if (headerEnd >= 0) {
                ...
                this.responseReadBuffer = Buffer.from(this.responseReadBuffer.subarray(headerEnd+4));
                ...
                if (responseHeader.statusCode === undefined) {
                    this.logger.warn(`Could not find status header in HTTP response ${JSON.stringify(responseHeader)}`);
                ...
                if (headers.has('transfer-encoding')) {
                    ...
                    this.responseChunkRemainder = parseInt(headers.get('content-length') || "-1")
                } else {
                    this.responseState = ResponseState.RECEIVING_BODY_CHUNKED;
                    this.responseChunkRemainder = 0;
                }
                const responseBody = new ReadableStream<Uint8Array>({
                    pull: controller => {
                        if (this.error || this.responseState == ResponseState.RECEIVING_COMPLETED) {
                ...
                this.resolve(new Response(responseBody, {
    ...
    private pull(controller: ReadableStreamDefaultController<Uint8Array>): boolean {
        if (this.responseState == ResponseState.RECEIVING_COMPLETED) {
            controller.close();
            return false;
        }
                ...
                if (this.responseChunkRemainder == 0) {
                    ...
                    while (this.responseReadBuffer.length >= 2 && chunkHeaderEnd == 0) {
                        ...
                        if (chunkHeaderEnd == 0) {
                            ...
                            this.responseReadBuffer = Buffer.from(this.responseReadBuffer.subarray(2));
                        }
                    }
                    if (chunkHeaderEnd > 0) {
                        this.responseChunkRemainder = parseInt(Buffer.from(this.responseReadBuffer, 0, chunkHeaderEnd).toString('utf8'), 16);
                        this.responseReadBuffer = Buffer.from(this.responseReadBuffer.subarray(chunkHeaderEnd + 2));
                        if (this.responseChunkRemainder <= 0) {
                            this.responseState = ResponseState.RECEIVING_COMPLETED;
                            controller.close();
                            return true;
                        ...
                        this.error = "Cannot find chunked encoding header within 64 bytes, given up";
                        controller.error(this.error);
                    }
                }
                if (this.responseChunkRemainder > 0 && this.responseReadBuffer.length > 0) {
                    const chunk = Buffer.from(this.responseReadBuffer.subarray(0, this.responseChunkRemainder));
                    this.responseReadBuffer = Buffer.from(this.responseReadBuffer.subarray(this.responseChunkRemainder));
