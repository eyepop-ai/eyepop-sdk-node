...
export class RenderBox implements Render {
    ...
    public draw(element: PredictedObject, xOffset: number, yOffset: number, xScale: number, yScale: number, streamTime: StreamTime, color?: string): void {
        ...
        const lineWidth = Math.max(canvasDimension * 0.0025, style.scale)
    ...
    createCornerPoints(x: number, y: number, w: number, h: number, cornerSize: number, padding: number = 1, insetPadding: number = 1): Array<Array<{ x: number; y: number }>> {
        ...
        return [
            ...
            [
                ...
                {
                    x: adjustedX + insetPadding,
                    y: adjustedY + adjustedH - innerCornerSize - insetPadding,
                },
                {
                    x: adjustedX + insetPadding,
                    y: adjustedY + adjustedH - insetPadding,
                },
                {
                    x: adjustedX + innerCornerSize,
                    y: adjustedY + adjustedH - insetPadding,
                },
            ],
            // Top right corner
            [
                ...
                {
                    x: adjustedX + adjustedW - innerCornerSize - insetPadding,
                    y: adjustedY + insetPadding,
                },
                {
                    x: adjustedX + adjustedW - insetPadding,
                    y: adjustedY + insetPadding,
                },
                {
                    x: adjustedX + adjustedW - insetPadding,
                    y: adjustedY + innerCornerSize,
                },
            ],
            // Bottom right corner
            [
