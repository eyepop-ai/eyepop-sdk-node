...
export let authenticateBrowserSession: (auth0: Auth0Options, eyepopUrl: string) => Promise<Session>

if ('document' in globalThis && 'implementation' in globalThis.document) {
    authenticateBrowserSession = async (auth0: Auth0Options, eyepopUrl: string) => {
        ...
        const session: Session = {
            eyepopUrl: eyepopUrl, accessToken: accessToken, validUntil: Date.now() + (60 * 60 * 1000)
        }
        return session
    }
} else {
